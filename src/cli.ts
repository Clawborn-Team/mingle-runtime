...
function renderOnboardingGuide(b: InstalledBinding, installUrl: string | undefined, configPath: string): string {
  ...
  return [
    ...
    ``,
    `STEP 3 — MIRROR your identity up to Mingle (just so the OWNER sees you by name in`,
    `their own console — a Local Agent is NOT publicly discoverable, so there is no`,
    `"looking_for"/who-to-meet). Read the api-key from your config (do NOT retype/print it):`,
    `  ${keyExpr}`,
    `  curl -s -X PATCH ${b.imUrl}/v1/me -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \\`,
    `    -d '{"display_name":"<name>","interests":["<...>"]}'`,
    ``,
